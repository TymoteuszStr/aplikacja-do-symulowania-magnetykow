...
XYSimulation::XYSimulation(int latticeN, int size, int W, int H, double temp, int FPS, int j, bool showOnL,bool showA,bool showL,bool showF, QWidget *parent) : QWidget(parent)
{
...
void XYSimulation::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
{    
    ...
    {
        fieldValue[i] = 0.0;
        latticeData->spin[i].theta = randomNumber(0.0, 2.0*M_PI);
    }
...
void XYSimulation::stepMC()
{
    ...
    double dE=0.0;
    double dTheta=randomNumber(-0.5,0.5);
...
void XYSimulation::calculate()
{
    ...
    if(counterCalc == maxIteration+startPoint)
    {
        ...
        saveOutput("./save/XYSimulation/default.txt");
    }


}

void XYSimulation::drawLattice()
{
...
void XYSimulation::drawOnLattice()
{
    QPainter painter(this);
    ...
    for(int i =0; i < latticeData->getNumberOfNodes(); i++)
    {
        ...
        else newColor = QColor(255,255,255);
...
void XYSimulation::setArrow(double x0, double y0, double x1, double y1, double x)
{
    QPainter painter(this);
    ...
    painter.drawLine(
                static_cast<int>(x1 + cos(M_PI - x + M_PI/6)*l),
                static_cast<int>(y1 + sin(M_PI - x + M_PI/6)*l),
                static_cast<int>(x1),
                static_cast<int>(y1));

    painter.drawLine(
                static_cast<int>(x1 + cos(-M_PI/2 - x + M_PI/6)*l),
                static_cast<int>(y1 + sin(-M_PI/2 - x + M_PI/6)*l),
                static_cast<int>(x1),
                static_cast<int>(y1));
...
void XYSimulation::drawNeighbour()
{
    QPainter painter(this);
...
void XYSimulation::uniformField()
{
    for(int i = 0; i < latticeData->getNumberOfNodes(); i++)
    {
...
void XYSimulation::nonUniformField()
{
...
double XYSimulation::randomNumber(double a, double b)
{
    double u = static_cast<double>(rand()%RAND_MAX)/RAND_MAX;
    return a+(b-a)*u; // losowe zmienne od a do b
}

void XYSimulation::mousePressEvent(QMouseEvent *event)
{
    Q_UNUSED(event)
...
void XYSimulation::turnOnField(double value, double direction)
{
...
void XYSimulation::turnOffField()
{
    uniformFieldValue = 0;
    uniformField();
}
void XYSimulation::saveOutput(QString path)
{
...
QString XYSimulation::generateMsg()
{
